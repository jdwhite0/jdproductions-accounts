...
async function maybeSendInvoice(event, repo, options, positionId) {
  ...
  try {
    ...
    // Ledger is already written. A thrown invoice error must not 500 the
    // webhook or Stripe retries the same paid event until the endpoint is disabled.
    console.error('early support invoice send failed', err);
    return {
      skipped: true,
      reason: 'invoice_send_failed',
      error: err?.code || err?.message || 'invoice_send_failed'
    };
  }
}
