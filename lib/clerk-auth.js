...
/** Short-lived token for same-site invest restore. Consumed immediately. */
export const SIGN_IN_TOKEN_TTL_SECONDS = 60;

/**
 * Mint a one-time Clerk sign-in token for an already-authenticated user.
 * Consume-only — does not configure Clerk. Do not log the token.
 */
export async function mintSignInTokenForUser(
  clerkUserId,
  { createSignInToken } = {},
) {
  if (!clerkUserId) {
  ...
  const create =
    ...
    (async (params) => {
      ...
      return clerk.signInTokens.createSignInToken(params);
    });
  ...
  if (!token) {
    const err = new Error("Could not mint ticket");
    err.status = 503;
    err.code = "ticket_mint_failed";
    throw err;
  }
  return token;
}
