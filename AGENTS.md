...
|---|---|---|---|---|
| 1 | **Shared Clerk identity** | Same Clerk application / pool as ACCESS, via `VITE_CLERK_PUBLISHABLE_KEY`. Sign-in/up embed on this origin (`allowed_origins`); not a Clerk satellite (accounts.* cannot be registered). | consume only | Publishable key is public. Secret key, if ever added, is server-only (`CLERK_SECRET_KEY`, never `VITE_`) |
...
- `ClerkProvider` in `src/main.jsx` must keep `signInUrl="/auth/login"`,
  ...
  set `isSatellite` — Clerk cannot register `accounts.*` as a satellite
  (`reserved_subdomain` / plan), and satellite mode without a registered
  domain causes a stuck “Redirecting to sign in…” loop.
- Live routes that must keep working: `/auth/login`, `/auth/register`,
  `/auth/bridge`. Login and register embed Clerk `<SignIn>` / `<SignUp>`
  (hash routing) on this origin. Bridge payload is unchanged.
