...
import { getWebhookSecret, getStripe } from '../../lib/stripe.js';
...
import { sendItemizedInvoice } from '../../lib/early-support/invoice.js';
...
export async function POST(request) {
  try {
    ...
    const result = await handleStripeWebhookRequest({
      ...
      processEvent: (event) =>
        processStripeEvent(event, repo, {
          sendInvoice: (ctx) => sendItemizedInvoice({ ...ctx, stripe: getStripe() })
        })
    });
